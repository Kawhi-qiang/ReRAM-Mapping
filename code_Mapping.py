...
Modified by：
    Kawhi qiang - zyqiang20221209@163.com
...
class SIMPLER_Top_Data_Structure:
    ...
    def RunAlgorithm(self):
            global END_OF_LINE_OUTPUT, find_mapping_first ###    
            ...
            if SORT_ROOTS == 'NO':
                for r in ROOTs:    
                    ...
                    elif (find_mapping_first==0):###
                        find_mapping_first=1
                t2 = time.time()
...
def SIMPLER_Main (BenchmarkStrings, Max_num_gates, ROW_SIZE, Benchmark_name, generate_json, print_mapping, print_warnings, end_of_line_output):
    global JSON_CODE_GEN, PRINT_CODE_GEN, PRINT_WARNING, SORT_ROOTS, END_OF_LINE_OUTPUT, find_mapping_first
    ...
    find_mapping_first = 0  ###
    
    for Row_size in ROW_SIZE: 
      if (find_mapping_first==0): ### if it finds minimum cells to map, then program stops
