...
def main():
    ...
    ROW_SIZE =[int(i) for i in list(range(27,200))]  ###
    #ROW_SIZE =[40]
    ...
    elif input_format == 'pla':
        abc_script = abc_script.replace('read_blif', 'read_pla')
    abc_script = abc_script.replace('lib.genlib', 'mcnc1_nor2.genlib')
    #abc_output_path = tempfile.mktemp()
    abc_script = abc_script.replace('output.v', output_path)
    ...
    SIMPLER_Mapping.SIMPLER_Main([output_path], Max_num_gates, ROW_SIZE, input_path.split(".")[0], generate_json, print_mapping, print_warnings, end_of_line_output)
